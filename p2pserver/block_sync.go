...
func (this *BlockSyncMgr) checkTimeout() {
	...
	for height, flightInfo := range headerTimeoutFlights {
		...
		msg := msgpack.ConstructHeadersReqMsg(headerHash)
	...
	for blockHash, flightInfo := range blockTimeoutFlights {
		...
		msg := msgpack.ConstructBlkDataReq(blockHash)
		err := this.server.Send(reqNode, msg, false)
		if err != nil {
...
func (this *BlockSyncMgr) syncBlock() {
	...
	for {
		...
		msg := msgpack.ConstructBlkDataReq(nextBlockHash)
		err := this.server.Send(reqNode, msg, false)
		if err != nil {
...
func (this *BlockSyncMgr) saveBlock() {
	...
	for {
		...
		if err != nil {
			...
			msg := msgpack.ConstructBlkDataReq(nextBlock.Hash())
			err := this.server.Send(reqNode, msg, false)
			if err != nil {
